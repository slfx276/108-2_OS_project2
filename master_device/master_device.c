...
// device information
static struct miscdevice master_dev = { // declare a structure
	/* this is one of the initialization methods */
...
{
	long ret = -EINVAL;
	// size_t data_size = 0, offset = 0; // delete?
	char *tmp;
	pgd_t *pgd;
	pud_t *pud;
	pmd_t *pmd;
...
{
	int ret;
	ret = remap_pfn_range(vma, vma->vm_start, virt_to_phys(filp->private_data)>>PAGE_SHIFT, vma->vm_end - vma->vm_start, vma->vm_page_prot);
	if (ret < 0) {
